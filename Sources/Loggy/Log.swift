...
// MARK: - Public Sugar (`Log`)

/// Convenience façade so users can type `Log.d(...)` instead of `Loggy.d(...)`.
public enum Log {
    ...
    /// Example:
    /// ```swift
    /// Log.w("API response took longer than expected")
    ...
    /// Example:
    /// ```swift
    /// Log.e("Failed to parse JSON response: \(error.localizedDescription)")
    ...
    /// Example:
    /// ```swift
    /// Log.wtf("This code path should never execute")
